# PowerShell script to create repair-service structure
# This script creates all folders and files for the repair-service without any code content

$serviceName = "repair-service"
$basePackagePath = "org/chiro/repair"
...
$commandFiles = @(
    "CreateRepairOrderCommand.kt",
    "UpdateRepairOrderStatusCommand.kt",
    "RecordRepairActivityCommand.kt",
    "ConsumePartsForRepairCommand.kt",
    "LogLaborForRepairCommand.kt",
...
$eventFiles = @(
    "CRMCustomerIssueReportedEventDTO.kt",
    "ProductRepairableItemEventDTO.kt",
    "InventoryPartsAvailableEventDTO.kt",
    "SalesOrderWarrantyInfoEventDTO.kt"
)
...
$queryFiles = @(
    "RepairOrderDetailsDTO.kt",
    "RepairProgressHistoryDTO.kt",
    "WarrantyClaimDetailsDTO.kt",
    "RepairCostAnalysisReportDTO.kt"
)
...
$responseFiles = @(
    "RepairOrderCreationResponse.kt"
)
...
$applicationServiceFiles = @(
    "RepairOrderApplicationService.kt",
    "RepairProgressApplicationService.kt",
    "PartsAndLaborConsumptionApplicationService.kt",
    "WarrantyManagementApplicationService.kt",
    "RepairCostingApplicationService.kt"
)
...
$domainEventFiles = @(
    "RepairOrderCreatedEvent.kt",
    "RepairOrderProgressedEvent.kt",
    "PartsConsumedForRepairEvent.kt",
    "LaborLoggedForRepairEvent.kt",
    "RepairOrderCompletedEvent.kt",
    "WarrantyClaimSubmittedEvent.kt",
    "WarrantyClaimApprovedEvent.kt"
)
...
$exceptionFiles = @(
    ...
    "InvalidRepairOrderStatusException.kt",
    "WarrantyClaimNotFoundException.kt",
    "InvalidWarrantyClaimException.kt",
    "InsufficientPartsForRepairException.kt"
)
...
$repositoryFiles = @(
    "RepairOrderRepository.kt",
    "WarrantyClaimRepository.kt"
)
...
$domainServiceFiles = @(
    "RepairCostCalculationService.kt",
    "WarrantyValidationService.kt",
    "RepairWorkflowService.kt"
)
...
New-Item -ItemType File -Path "$mainResourcesDir/data/default-repair-templates.json" -Force | Out-Null
...
New-Item -ItemType File -Path "$testKotlinDir/application/service/RepairApplicationServiceTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/domain/aggregate/RepairOrderTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/domain/aggregate/WarrantyClaimTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/domain/service/RepairCostCalculationServiceTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/domain/service/WarrantyValidationServiceTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/infrastructure/adapter/RepairResourceTest.kt" -Force | Out-Null

Write-Host ""
Write-Host "✅ Successfully created repair-service structure!" -ForegroundColor Green
Write-Host ""
...
Write-Host "   1. Update build.gradle.kts with repair service dependencies (Spring Boot, JPA, Kafka, etc.)"
Write-Host "   2. Configure application.properties with database and Kafka settings"
Write-Host "   3. Add database migration scripts for RepairOrder, WarrantyClaim, etc."
Write-Host "   4. Implement the Kotlin classes with proper repair service domain logic"
Write-Host "   5. Set up default-repair-templates.json seed data for common repair workflows"
Write-Host "   6. Configure warranty validation rules and repair pricing algorithms"

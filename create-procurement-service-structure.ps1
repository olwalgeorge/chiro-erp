# PowerShell script to create procurement-service structure
# This script creates all folders and files for the procurement-service without any code content

$serviceName = "procurement-service"
$basePackagePath = "org/chiro/procurement"
...
$commandFiles = @(
    "ApprovePurchaseRequisitionCommand.kt",
    "CreatePurchaseOrderCommand.kt",
    "CreatePurchaseRequisitionCommand.kt",
    "RecordGoodsReceiptCommand.kt",
    "UpdatePurchaseOrderCommand.kt",
...
$eventFiles = @(
    ...
    "GoodsReceivedEventDTO.kt",
    "FinanceVendorPaymentProcessedEventDTO.kt"
)
...
$queryFiles = @(
    "PurchaseOrderDetailsDTO.kt",
    "PurchaseRequisitionDetailsDTO.kt",
    "VendorDetailsDTO.kt",
    "ProcurementAnalyticsReportDTO.kt",
    "GoodsReceiptSummaryDTO.kt"
)
...
$responseFiles = @(
    "POCreationResponse.kt",
    "RequisitionApprovalResponse.kt"
)
...
$incomingPortFiles = @(
    "ProcurementCommandPort.kt",
    "ProcurementQueryPort.kt",
...
$applicationServiceFiles = @(
    "PurchaseOrderApplicationService.kt",
    "PurchaseRequisitionApplicationService.kt",
    "GoodsReceiptApplicationService.kt",
    "VendorManagementApplicationService.kt",
    "ProcurementAnalyticsApplicationService.kt"
)
...
$exceptionFiles = @(
    "InsufficientRequisitionApprovalException.kt",
    "InvalidPurchaseOrderStatusException.kt",
    "PurchaseOrderNotFoundException.kt",
    "PurchaseRequisitionNotFoundException.kt",
...
$repositoryFiles = @(
    ...
    "PurchaseRequisitionRepository.kt",
    "VendorRepository.kt"
)
...
$domainServiceFiles = @(
    "RequisitionApprovalService.kt",
    "SourcingService.kt",
    "ThreeWayMatchingService.kt"
)
...
$valueObjectFiles = @(
    ...
    "ProductId.kt",
    "PurchaseOrderId.kt",
    "PurchaseOrderStatus.kt",
    "PurchaseRequisitionId.kt",
    "Quantity.kt",
    "ReceivingStatus.kt",
    "UnitOfMeasure.kt",
    "VendorId.kt",
...
$incomingAdapterFiles = @(
    "ProcurementGraphQLResource.kt",
    "ProcurementResource.kt",
...
$outgoingAdapterFiles = @(
    "JpaGoodsReceiptRepository.kt",
    "JpaPurchaseOrderRepository.kt",
    "JpaPurchaseRequisitionRepository.kt",
    "JpaVendorRepository.kt",
    "KafkaFinanceEventPublisher.kt",
    "KafkaInventoryEventPublisher.kt",
    "KafkaProcurementEventPublisher.kt",
    "NotificationServiceClient.kt",
    "ExternalSupplierIntegrationClient.kt"
)
...
New-Item -ItemType File -Path "$mainResourcesDir/data/default-vendors.json" -Force | Out-Null
...
New-Item -ItemType File -Path "$testKotlinDir/application/service/ProcurementApplicationServiceTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/domain/aggregate/PurchaseOrderTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/domain/aggregate/PurchaseRequisitionTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/domain/service/ThreeWayMatchingServiceTest.kt" -Force | Out-Null
New-Item -ItemType File -Path "$testKotlinDir/infrastructure/adapter/ProcurementResourceTest.kt" -Force | Out-Null

Write-Host ""
Write-Host "✅ Successfully created procurement-service structure!" -ForegroundColor Green
Write-Host ""
...
Write-Host "   1. Update build.gradle.kts with procurement-specific dependencies (Spring Boot, JPA, Kafka, etc.)"
Write-Host "   2. Configure application.properties with database and Kafka settings"
Write-Host "   3. Add database migration scripts for PurchaseOrder, PurchaseRequisition, Vendor, etc."
Write-Host "   4. Implement the Kotlin classes with proper procurement domain logic"
Write-Host "   5. Set up default-vendors.json seed data"
